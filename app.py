...
df = pd.read_sql("SELECT * FROM public.trades", engine.connect(), parse_dates=('Entry time'))
...
def filter_df(df, exchange, leverage, start_date, end_date):
    df1 = df.loc[(df['Exchange']==exchange) & (df['Margin']==int(leverage)) & (df['Entry time']>=start_date)& (df['Entry time']<=end_date)]
    return df1
...
@app.callback(
    [
        ...
        dash.dependencies.Output('strat-vs-market-div', 'children'),
    ],
    (
...
def update_daterange(value):
    df_exselect = df[df['Exchange']==value]
    return df_exselect['Entry time'].min()
...
def update_graph_date(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    b1=dff[dff['Trade type']=='Short']
    b2=dff[dff['Trade type']=='Long']

    fig = go.Figure([go.Bar(x=b1['Entry time'], y= b1['Pnl (incl fees)'],name='Short',width=3*3600*24*1000),
                     go.Bar(x=b2['Entry time'], y= b2['Pnl (incl fees)'],name='Short',width=3*3600*24*1000),
                    ],
    go.Layout(height=450)
    )
    return fig
...
def update_btc_line(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    return {
        'data': [go.Scatter(
            x=dff['Entry time'],
